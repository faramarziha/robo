...
if (isset($update['message']['successful_payment'])) {
    ...
    if (!empty($Payment_report) && $Payment_report['payment_Status'] != "paid") {
        DirectPayment($order_id, "images.jpg");
        update("Payment_report", "payment_Status", "paid", "id_order", $order_id);
        sendmessage($from_id, "✅ پرداخت شما با استارز تلگرام با موفقیت انجام شد و سرویس فعال گردید.", null, 'HTML');
    }
    return;
}
...
if ($user['step'] == "createusertest" || preg_match('/locationtest_(.*)/', $datain, $dataget) || ($text == $textbotlang['textbot']['userTest'] || $datain == "usertestbtn" || $text == "usertest")) {
    ...
    if ($datain == "cart_to_offline") {
        ...
        $minbalancestars = intval(select("PaySetting", "ValuePay", "NamePay", "minbalancestars", "select")['ValuePay'] ?? 1);
        $maxbalancestars = intval(select("PaySetting", "ValuePay", "NamePay", "maxbalancestars", "select")['ValuePay'] ?? 10000);
...
if (isset($update['message']['successful_payment'])) {
    ...
    // Check invoice expiry — if expired, refundStarPayment and notify user
    $expiresAt = intval($Payment_report['expires_at'] ?? 0);
    if ($expiresAt > 0 && time() > $expiresAt) {
        update("Payment_report", "payment_Status", "expire", "id_order", $Payment_report['id_order']);
        // Refund the Stars payment
