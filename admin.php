...
elseif ($datain == "systemsms") {
    ...
    $stmt->execute();
    $countpay = count($stmt->fetchAll(PDO::FETCH_ASSOC));
    ...
    try {
        ...
        $claimStmt->bindValue(':id_order', $order_id, PDO::PARAM_STR);
        $claimStmt->execute();
        $currentPaymentStatus = $claimStmt->fetchColumn();

        if ($currentPaymentStatus === false
            || $currentPaymentStatus === 'paid'
            || $currentPaymentStatus === 'reject'
            || !settleOrderOnce($order_id)) {
            $pdo->rollBack();
            telegram('answerCallbackQuery', [
                'callback_query_id' => $callback_query_id,
                'text' => $textbotlang['Admin']['Payment']['reviewedPayment'],
        ...
        error_log("admin.php confirm payment: settlement claim failed for order {$order_id}: " . $e->getMessage());
    ...
    try {
        ...
        error_log("admin.php confirm payment: DirectPayment failed after claim for order {$order_id}: " . $e->getMessage());
        sendmessage($from_id, $textbotlang['Admin']['errorOccurred'], $keyboardadmin, 'HTML');
        return;
    }

    $cashbackSetting = select("PaySetting", "ValuePay", "NamePay", "chashbackcart", "select");
    $cashbackPercent = (float) ($cashbackSetting['ValuePay'] ?? 0);
    if ($cashbackPercent > 0 && $cashbackPercent <= 100) {
        $cashbackAmount = (int) round(
            ((float) $Payment_report['price'] * $cashbackPercent) / 100
        );

        if ($cashbackAmount > 0) {
            $cashbackBalance = creditBalance((int) $Payment_report['id_user'], $cashbackAmount);
            $cashbackUserExists = $cashbackBalance !== 0
                || select("user", "id", "id", $Payment_report['id_user'], "select");
            if (!$cashbackUserExists) {
                error_log("admin.php confirm payment: cashback credit failed for order {$order_id}");
            } else {
                $text_report = sprintf($textbotlang['users']['Balance']['giftDepositAlt'], $cashbackAmount);
    ...
    if (!$Payment_report || $Payment_report['payment_Status'] !== 'reject') {
        ...
        sendmessage(
            $from_id,
            $textbotlang['Admin']['Payment']['reviewedPayment'],
            $keyboardadmin,
            'HTML'
        );
        return;
    ...
    $amountToAdd = (isset($userdata['price']) && is_numeric($userdata['price'])) ? (int) $userdata['price'] : null;
