...
if (!empty($users)) {
    ...
    $stmts = $pdo->prepare("SELECT * FROM invoice WHERE id_user IN ($placeholders) AND (Status = 'active' OR Status = 'end_of_time'  OR Status = 'end_of_volume' OR status = 'sendedwarn' OR Status = 'send_on_hold')");
    ...
    if (!empty($invoices_to_update)) {
        ...
        $updateStmt = $pdo->prepare("UPDATE invoice SET Status = 'disablebyadmin' WHERE id_invoice IN ($updatePlaceholders)");
