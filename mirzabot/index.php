...
// impersonate any user id -- including an admin's.
// Checked before botapi.php so a forged request is dropped before it can touch
// the database. Empty secret = fail open, so an install whose config.php
// predates this variable keeps working until 'mirza repair' regenerates it.
if (!empty($webhook_secret)) {
    $sent_secret = $_SERVER['HTTP_X_TELEGRAM_BOT_API_SECRET_TOKEN'] ?? '';
    // hash_equals: a plain != leaks the secret one byte at a time by timing.
    if (!is_string($sent_secret) || !hash_equals($webhook_secret, $sent_secret)) {
...
#-----------Telegram Stars payment (XTR)------------#
# NOTE: must run after $setting/$ManagePanel/$user/$keyboard are initialised,
# because DirectPayment() pulls them in via `global`.
if (isset($update['pre_checkout_query'])) {
    ...
    if ($Payment_report == false) {
        telegram('answerPreCheckoutQuery', [
            ...
            'ok' => false,
            'error_message' => 'فاکتور پیدا نشد. لطفاً دوباره سفارش بدهید.',
        ]);
        return;
    }
    if ($Payment_report['payment_Status'] == "paid") {
        telegram('answerPreCheckoutQuery', [
            'pre_checkout_query_id' => $update['pre_checkout_query']['id'],
            'ok' => false,
            'error_message' => 'این فاکتور قبلاً پرداخت شده است.',
        ]);
        return;
    }
    ...
    if ($expiresAt > 0 && time() > $expiresAt) {
        ...
        telegram('answerPreCheckoutQuery', [
            'pre_checkout_query_id' => $update['pre_checkout_query']['id'],
            'ok' => false,
            'error_message' => '⏰ مهلت این فاکتور به پایان رسیده است. لطفاً مجدداً سفارش بدهید.',
        ]);
        return;
    }
    telegram('answerPreCheckoutQuery', [
...
if (isset($update['message']['successful_payment'])) {
    ...
    $id_order = $sp['invoice_payload'];
    $total_stars = $sp['total_amount'] ?? 0; // actual Stars paid
    $Payment_report = select("Payment_report", "*", "id_order", $id_order, "select");
    if ($Payment_report == false) {
        return;
    }
    ...
    // Check invoice expiry — if expired, refund the Stars and notify the user
    $expiresAt = intval($Payment_report['expires_at'] ?? 0);
    if ($expiresAt > 0 && time() > $expiresAt) {
        update("Payment_report", "payment_Status", "expire", "id_order", $Payment_report['id_order']);
    ...
    if (strlen($setting['Channel_Report'] ?? '') > 0) {
        $reportText = sprintf(
            $textbotlang['Admin']['reportgroup']['newPaymentStar'] ?? "⭐️ پرداخت جدید Stars\n👤 کاربر: %s (ID: %s)\n💰 مبلغ: %s تومان\n⭐️ استارز پرداختی: %s",
            $Balance_id['username'] ?? '',
            $Balance_id['id'] ?? '',
        ...
        telegram('sendmessage', [
            'chat_id' => $setting['Channel_Report'],
            'message_thread_id' => $paymentreports ?? null,
            'text' => $reportText,
            'parse_mode' => "HTML"
        ]);
    }
...
if ($user['step'] == "createusertest" || preg_match('/locationtest_(.*)/', $datain, $dataget) || ($text == $textbotlang['textbot']['userTest'] || $datain == "usertestbtn" || $text == "usertest")) {
    ...
    if (($locationproduct)->rowCount() == 1) {
        ...
        if ($setting['statuscategory'] == "offcategory") {
            ...
            if ($setting['statuscategorygenral'] == "oncategorys") {
                ...
                $params = [':location' => $location, ':agent' => $user['agent']];
                $marzban_list_get = select("marzban_panel", "*", "name_panel", $location, "select");
                ...
                if ($datain == "buy") {
                    Editmessagetext($from_id, $message_id, $textproduct, KeyboardProduct($marzban_list_get['name_panel'], $query, $user['pricediscount'], $datakeyboard, $statuscustom, "backuser", null, "customsellvolume", $params));
                } else {
                    sendmessage($from_id, $textproduct, KeyboardProduct($marzban_list_get['name_panel'], $query, $user['pricediscount'], $datakeyboard, $statuscustom, "backuser", null, "customsellvolume", $params), 'HTML');
                }
            }
        } else {
            $nullproduct = select("product", "*", null, null, "count");
    ...
    if ($setting['statuscategory'] == "offcategory") {
        if ($setting['statuscategorygenral'] == "oncategorys") {
            ...
            Editmessagetext($from_id, $message_id, $textbotlang['users']['sell']['serviceSelect'], KeyboardProduct($marzban_list_get['name_panel'], $query, $user['pricediscount'], $datakeyboard, $statuscustom, $back, null, "customsellvolume", $params));
        }
    } else {
        $nullproduct = select("product", "*", null, null, "count");
        if ($nullproduct == 0) {
    ...
    if (isset($userdate['monthproduct'])) {
        $query = "SELECT * FROM product WHERE (Location = :location OR Location = '/all') AND agent = :agent AND category = :category AND Service_time = :service_time";
        $params = [':location' => $userdate['name_panel'], ':agent' => $user['agent'], ':category' => $categorynames, ':service_time' => $userdate['monthproduct']];
    } else {
        $query = "SELECT * FROM product WHERE (Location = :location OR Location = '/all') AND agent = :agent AND category = :category";
        $params = [':location' => $userdate['name_panel'], ':agent' => $user['agent'], ':category' => $categorynames];
    }
    ...
    Editmessagetext($from_id, $message_id, $textbotlang['users']['sell']['serviceSelectFirst'], KeyboardProduct($marzban_list_get['name_panel'], $query, $user['pricediscount'], $datakeyboard, $statuscustom, "backuser", null, "customsellvolume", $params));
    ...
    if ($setting['statuscategorygenral'] == "oncategorys") {
        ...
        $query = "SELECT * FROM product WHERE (Location = :location OR Location = '/all') AND agent = :agent AND Service_time = :service_time";
        $params = [':location' => $userdate['name_panel'], ':agent' => $user['agent'], ':service_time' => $monthenumber];
        $marzban_list_get = select("marzban_panel", "*", "name_panel", $userdate['name_panel'], "select");
        $statuscustomvolume = json_decode($marzban_list_get['customvolume'], true)[$user['agent']];
        if ($marzban_list_get['MethodUsername'] == $textbotlang['users']['customusername'] || $marzban_list_get['MethodUsername'] == $textbotlang['keyboard']['customUsernameRandom']) {
            $datakeyboard = "prodcutservices_";
    ...
    $query = "SELECT * FROM product WHERE (Location = :location OR Location = '/all') AND agent = :agent";
    $params = [':location' => $location, ':agent' => $user['agent']];
    Editmessagetext($from_id, $message_id, $textbotlang['users']['sell']['serviceSelect'], KeyboardProduct($marzban_list_get['name_panel'], $query, $user['pricediscount'], $datakeyboard, $statuscustom, "backuser", null, "customsellvolumeom", $params));
    ...
    if ($datain == "cart_to_offline") {
        ...
        if ($user['Processing_value'] < 1000) {
            sendmessage($from_id, "❌ حداقل مبلغ قابل پرداخت ۱,۰۰۰ تومان می‌باشد.", null, 'HTML');
            return;
        }
        $mainbalance = select("PaySetting", "ValuePay", "NamePay", "minbalancetetrapay", "select")['ValuePay'] ?? 1000;
        $maxbalance = select("PaySetting", "ValuePay", "NamePay", "maxbalancetetrapay", "select")['ValuePay'] ?? 10000000;
        if ($user['Processing_value'] < $mainbalance || $user['Processing_value'] > $maxbalance) {
            sendmessage($from_id, strtr($textbotlang['users']['Balance']['depositRange'], ['{mainbalance}' => number_format($mainbalance), '{maxbalance}' => number_format($maxbalance)]), null, 'HTML');
            return;
        }
        deletemessage($from_id, $message_id);
        sendmessage($from_id, $textbotlang['users']['Balance']['linkpayments'], $keyboard, 'HTML');
        ...
        $dateacc = date('Y/m/d H:i:s');
        $randomString = bin2hex(random_bytes(5));
        $pay = createPayTetraPay($user['Processing_value'], $randomString);
        if (empty($pay['ok']) && (string)($pay['status'] ?? '') !== '100') {
            sendmessage($from_id, "❌ خطا در ساخت لینک پرداخت تتراپی. لطفاً کلید API را بررسی کرده یا بعداً تلاش کنید.", $keyboard, 'HTML');
            step('home', $from_id);
            return;
        }
        // TetraPay API returns payment_url_web and payment_url_bot (not 'web')
        $payUrl = $pay['payment_url_web'] ?? $pay['payment_url_bot'] ?? $pay['web'] ?? '';
        // Calculate expiry timestamp
        $tetraExpiryMinutes = intval(select("PaySetting", "ValuePay", "NamePay", "tetrapay_invoice_expiry", "select")['ValuePay'] ?? 30);
        $expiresAt = time() + ($tetraExpiryMinutes * 60);
        ...
        $stmt->execute([$from_id, $randomString, $dateacc, $user['Processing_value'], "Unpaid", "tetrapay", $invoice, $expiresAt]);
        $paymentkeyboard = json_encode([
            'inline_keyboard' => [
                [
                    ['text' => "🌐 ورود به درگاه تتراپی", 'url' => $payUrl],
                ]
            ]
        ]);
        $price_format = number_format($user['Processing_value'], 0);
        ...
        $message_id = sendmessage($from_id, $textnowpayments, $paymentkeyboard, 'HTML');
        updatePaymentMessageId($message_id, $randomString);
    } elseif ($datain == "telegram_stars") {
        $stars_rate = getStarPriceToman();
        $amount_toman = intval($user['Processing_value']);
        $stars_needed = max(1, (int)ceil($amount_toman / max(1, $stars_rate)));
        // Amount limits are stored in TOMAN, exactly like every other gateway.
        $minbalancestars = intval(select("PaySetting", "ValuePay", "NamePay", "minbalancestars", "select")['ValuePay'] ?? 20000);
        $maxbalancestars = intval(select("PaySetting", "ValuePay", "NamePay", "maxbalancestars", "select")['ValuePay'] ?? 1000000);
        if ($amount_toman < $minbalancestars || $amount_toman > $maxbalancestars) {
            sendmessage($from_id, strtr($textbotlang['users']['Balance']['depositRange'], ['{mainbalance}' => number_format($minbalancestars), '{maxbalance}' => number_format($maxbalancestars)]), null, 'HTML');
            return;
        }
        // Separate ceiling on the star COUNT (not the toman amount). Telegram
        // rejects invoices above its own per-invoice limit, so fail here with a
        // readable message instead of surfacing an opaque sendInvoice error.
        $starsMaxCount = intval(select("PaySetting", "ValuePay", "NamePay", "stars_max_count", "select")['ValuePay'] ?? 100000);
        if ($starsMaxCount > 0 && $stars_needed > $starsMaxCount) {
            sendmessage($from_id, strtr($textbotlang['users']['Balance']['starsMaxCount'], ['{stars}' => number_format($stars_needed), '{max}' => number_format($starsMaxCount)]), null, 'HTML');
            return;
        }
        deletemessage($from_id, $message_id);
        $randomString = bin2hex(random_bytes(5));
        ...
        $invoice = "{$user['Processing_value_tow']}|{$user['Processing_value_one']}";
        // Calculate expiry timestamp
        $starsExpiryMinutes = intval(select("PaySetting", "ValuePay", "NamePay", "stars_invoice_expiry", "select")['ValuePay'] ?? 30);
        $expiresAt = time() + ($starsExpiryMinutes * 60);
        $stmt = $pdo->prepare("INSERT INTO Payment_report (id_user,id_order,time,price,payment_Status,Payment_Method,id_invoice,expires_at,dec_not_confirmed) VALUES (?,?,?,?,?,?,?,?,?)");
        $stmt->execute([$from_id, $randomString, $dateacc, $user['Processing_value'], "Unpaid", "telegram_stars", $invoice, $expiresAt, json_encode(['stars_count' => $stars_needed, 'stars_rate' => $stars_rate])]);
        $gethelp = select("PaySetting", "ValuePay", "NamePay", "helpstar", "select")['ValuePay'] ?? '2';
        if ($gethelp != 2 && $gethelp != 0) {
            $data = json_decode($gethelp, true);
            if (is_array($data)) {
                if ($data['type'] == "text") {
                    sendmessage($from_id, $data['text'], null, 'HTML');
        ...
        sendStarsInvoice(
            $from_id,
            "⭐️ سفارش با استارز تلگرام",
            "پرداخت مبلغ " . number_format($amount_toman) . " تومان ($stars_needed استارز)",
            $randomString,
            $stars_needed
        );
...
# NOTE: pre_checkout_query / successful_payment are handled near the top of this
# file, right after $setting/$ManagePanel/$user are initialised.
if (preg_match('/extends_(\w+)_(.*)/', $datain, $dataget)) {
    $username = $dataget[1];
    ...
    Editmessagetext($from_id, $message_id, $textbotlang['users']['extend']['selectservice'], KeyboardProduct($marzban_list_get['name_panel'], $query, $user['pricediscount'], "serviceextendselects-", false, "backuser", $username, "customsellvolume", $params));
