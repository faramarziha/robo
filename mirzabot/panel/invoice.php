...
try {
  ...
  if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    $exportInvoices = db_fetchAll($pdo, "SELECT * FROM invoice $whereSQL ORDER BY time_sell DESC", $params);
    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename=invoices_export_' . date('Y-m-d_H-i') . '.csv');
    $output = fopen('php://output', 'w');
    fputs($output, "\xEF\xBB\xBF"); // UTF-8 BOM
    fputcsv($output, ['ID Invoice', 'ID User', 'Username', 'Product', 'Price', 'Volume', 'Service Time', 'Status', 'Date']);
    foreach ($exportInvoices as $row) {
      fputcsv($output, [
        $row['id_invoice'] ?? '',
        $row['id_user'] ?? '',
        $row['username'] ?? '',
...
<div class="card fade-up">
  <div class="toolbar">
    ...
    <form method="GET" id="invoiceForm" class="toolbar-end">
      ...
      <?php endif; ?>
      <a href="invoice.php?export=csv<?= $search ? '&q=' . urlencode($search) : '' ?><?= $status ? '&status=' . urlencode($status) : '' ?>" class="btn-sec" style="font-size:.78rem;padding:6px 12px;display:inline-flex;align-items:center;gap:4px">
        📥 خروجی اکسل (CSV)
      </a>
    </form>
  </div>
