...
function select($table, $field, $whereField = null, $whereValue = null, $type = "select", $options = [])
{
    ...
    try {
        ...
        $result = ($type === "count") ? 0 : (($type === "fetchAll" || $type === "FETCH_COLUMN") ? [] : null);
    }
...
function createPayTetraPay($price, $order_id)
{
    global $domainhosts;
    $apiKey = select("PaySetting", "ValuePay", "NamePay", "merchant_id_tetrapay", "select")['ValuePay'] ?? '';
...
function sendStarsInvoice($chat_id, $title, $description, $payload, $stars_amount)
{
    return telegram('sendInvoice', [
        ...
        'currency' => 'XTR',
        'prices' => json_encode([
            ['label' => $title, 'amount' => intval($stars_amount)]
        ])
    ]);
}
function getLiveCryptoRatesToman()
{
    static $cachedRates = null;
    static $lastCheck = 0;
    if ($cachedRates !== null && (time() - $lastCheck) < 90) {
        return $cachedRates;
    }
    
    $ch = curl_init('https://apiv2.nobitex.ir/market/stats?srcCurrency=usdt,ton&dstCurrency=rls');
    ...
    if ($body !== false) {
        ...
        $usdtRial = $data['stats']['usdt-rls']['latest'] ?? null;
        $tonRial = $data['stats']['ton-rls']['latest'] ?? null;
        
        $usdtToman = (is_numeric($usdtRial) && $usdtRial > 0) ? (int) round(((float) $usdtRial) / 10) : null;
        $tonToman = (is_numeric($tonRial) && $tonRial > 0) ? (int) round(((float) $tonRial) / 10) : null;
        
        if ($usdtToman !== null || $tonToman !== null) {
            $cachedRates = [
                'usdt' => $usdtToman,
                'ton' => $tonToman
            ];
    ...
    return ['usdt' => null, 'ton' => null];
}

function getLiveUsdtRateToman()
{
    $rates = getLiveCryptoRatesToman();
    return $rates['usdt'];
}

function getLiveTonRateToman()
{
    $rates = getLiveCryptoRatesToman();
    return $rates['ton'];
}

function getStarPriceToman()
{
    $autoRateMode = select("PaySetting", "ValuePay", "NamePay", "stars_auto_rate", "select")['ValuePay'] ?? 'usdt';
    $marginPercent = floatval(select("PaySetting", "ValuePay", "NamePay", "stars_margin", "select")['ValuePay'] ?? '0');
    
    $basePrice = null;
    $rates = getLiveCryptoRatesToman();

    if ($autoRateMode == 'ton' && !empty($rates['ton'])) {
        // 1 Star is approximately 0.0033 TON
        $basePrice = (int) round($rates['ton'] * 0.0033);
    } elseif (($autoRateMode == 'usdt' || $autoRateMode == '1' || $autoRateMode == 'on') && !empty($rates['usdt'])) {
        // 1 Star is approximately 0.015 USDT
        $basePrice = (int) round($rates['usdt'] * 0.015);
    }

    if ($basePrice === null || $autoRateMode == 'off' || $autoRateMode == '0') {
        $basePrice = intval(select("PaySetting", "ValuePay", "NamePay", "stars_rate", "select")['ValuePay'] ?? 1000);
    }

    if ($marginPercent != 0) {
        $basePrice = (int) round($basePrice * (1 + ($marginPercent / 100)));
    }

    return max(1, $basePrice);
}

/**
 * Runs $fn inside a database transaction, rolling back on any error.
 * MySQL has no real nested transactions, so if one is already active the
 * callback is executed inline and the outer caller keeps ownership.
 */
function withTransaction(callable $fn)
{
    ...
    try {
        ...
        error_log('withTransaction rolled back: ' . $e->getMessage());
        throw $e;
    }
}

/**
 * Atomically adds $amount to a user's balance using a relative UPDATE.
 * This replaces the read-then-write pattern, which loses concurrent writes.
 * $amount may be negative to debit.
 */
function creditBalance($userId, $amount)
{
    global $pdo;

    $stmt = $pdo->prepare("UPDATE user SET Balance = COALESCE(Balance, 0) + :amount WHERE id = :id");
...
/**
 * Atomically debits only if the balance covers it. Returns false when the
 * funds are insufficient, so "check then spend" can never race.
 */
function debitBalanceIfSufficient($userId, $amount)
{
    global $pdo;
    ...
    $stmt = $pdo->prepare("UPDATE user SET Balance = Balance - :amount WHERE id = :id AND COALESCE(Balance, 0) >= :amount2");
    $stmt->execute([':amount' => $amount, ':id' => $userId, ':amount2' => $amount]);
    clearSelectCache('user');
...
/**
 * Idempotency guard for order settlement. The conditional UPDATE is the
...
function settleOrderOnce($orderId)
{
    ...
    $stmt = $pdo->prepare(
        "UPDATE Payment_report SET payment_Status = 'paid' WHERE id_order = :id AND payment_Status <> 'paid'"
    );
