...
try {
    ...
    if (!$table_exists) {
        ...
        $Check_filde = $pdo->query("SHOW COLUMNS FROM Payment_report LIKE 'expires_at'");
        if (($Check_filde)->rowCount() != 1) {
            $pdo->query("ALTER TABLE Payment_report ADD expires_at INT(11) NULL DEFAULT NULL");
            echo "The expires_at field was added ✅";
        }
    }
} catch (Exception $e) {
    file_put_contents('error_log', $e->getMessage());
...
try {
    ...
    $settings = [
        ...
        ['merchant_id_tetrapay', '0'],
        ['minbalancetetrapay', $main],
        ['maxbalancetetrapay', $max],
        ['chashbacktetrapay', '0'],
        ['helptetrapay', '2'],
        ['statusstars', 'offstars'],
        ['stars_rate', '2000'],
        ['chashbackstars', '0'],
        ['minbalancestars', $main],
        ['maxbalancestars', $max],
        ['stars_max_count', '100000'],
        ['stars_auto_rate', 'usdt'],
        ['stars_margin', '0'],
        ['stars_invoice_expiry', '30'],
        ['tetrapay_invoice_expiry', '30'],
        ['helpofflinearze', '2'],
    ...
    if (!$table_exists) {
        ...
        // Telegram Stars limit repair: earlier builds seeded minbalancestars=1 and
        // maxbalancestars=10000, but both keys are compared against the order
        // amount in TOMAN, so the 10000 ceiling rejected every realistic
        // subscription price. Only rows still holding the old defaults are
        // corrected, so values already tuned by an admin are left alone.
        $starsLimitFix = [
            ['maxbalancestars', '10000', (string) $max],
            ['minbalancestars', '1', (string) $main],
            ['stars_rate', '1000', '2000'],
        ];
        foreach ($starsLimitFix as $fix) {
            $__q6 = $pdo->prepare("UPDATE PaySetting SET ValuePay = ? WHERE NamePay = ? AND ValuePay = ?");
            $__q6->bindValue(1, $fix[2], PDO::PARAM_STR);
            $__q6->bindValue(2, $fix[0], PDO::PARAM_STR);
            $__q6->bindValue(3, $fix[1], PDO::PARAM_STR);
            $__q6->execute();
        }
