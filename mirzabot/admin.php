...
if (in_array($text, $textadmin) || $datain == "admin") {
    ...
    $sql1 = "SELECT COUNT(*) AS invoice_count FROM invoice WHERE (status = 'active' OR status = 'end_of_time' OR status = 'end_of_volume' OR status = 'sendedwarn' OR status = 'send_on_hold') AND name_product != :test_service_name";
    $stmt1 = $pdo->prepare($sql1);
    $stmt1->bindValue(':test_service_name', $textbotlang['common']['labels']['testServiceName'], PDO::PARAM_STR);
    $stmt1->execute();
    $invoiceactive = $stmt1->fetch(PDO::FETCH_ASSOC)['invoice_count'];
    $sqlall = "SELECT COUNT(*) AS invoice_count FROM invoice WHERE status != 'Unpaid' AND name_product != :test_service_name";
    $stmtall = $pdo->prepare($sqlall);
    $stmtall->bindValue(':test_service_name', $textbotlang['common']['labels']['testServiceName'], PDO::PARAM_STR);
    $stmtall->execute();
    $invoice = $stmtall->fetch(PDO::FETCH_ASSOC)['invoice_count'];
    $sql2 = "SELECT SUM(price_product) AS total_price FROM invoice WHERE (status = 'active' OR status = 'end_of_time' OR status = 'end_of_volume' OR status = 'sendedwarn' OR status = 'send_on_hold') AND name_product != :test_service_name";
    $stmt2 = $pdo->prepare($sql2);
    $stmt2->bindValue(':test_service_name', $textbotlang['common']['labels']['testServiceName'], PDO::PARAM_STR);
    $stmt2->execute();
    $invoicesum = $stmt2->fetch(PDO::FETCH_ASSOC)['total_price'];
    $sql33 = "SELECT SUM(price_product) AS total_price FROM invoice WHERE status!= 'Unpaid' AND name_product != :test_service_name";
    $stmt33 = $pdo->prepare($sql33);
    $stmt33->bindValue(':test_service_name', $textbotlang['common']['labels']['testServiceName'], PDO::PARAM_STR);
    $stmt33->execute();
    $invoiceSumRow = $stmt33->fetch(PDO::FETCH_ASSOC);
    ...
    $sql = "SELECT SUM(price_product) FROM invoice WHERE (time_sell BETWEEN :requestedDate AND :requestedDateend) AND (status = 'active' OR status = 'end_of_time'  OR status = 'end_of_volume' OR Status = 'send_on_hold' OR Status = 'sendedwarn') AND name_product != :test_service_name";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':requestedDate', $start_time_timestamp);
    $stmt->bindParam(':requestedDateend', $end_time_timestamp);
    $stmt->bindValue(':test_service_name', $textbotlang['common']['labels']['testServiceName'], PDO::PARAM_STR);
    $stmt->execute();
    ...
    $sql = "SELECT COUNT(*) AS count,SUM(price_product) as sum FROM invoice WHERE (time_sell BETWEEN :requestedDate AND :requestedDateend) AND Status != 'Unpaid'  AND name_product != :test_service_name";
    $stmt = $pdo->prepare($sql);
    $time_current = time();
    ...
    $sql = "SELECT COUNT(*) AS count FROM invoice WHERE (time_sell BETWEEN :requestedDate AND :requestedDateend)  AND name_product = :test_service_name";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':requestedDate', $desired_date_time_start);
    $stmt->bindParam(':requestedDateend', $time_current);
    $stmt->bindValue(':test_service_name', $textbotlang['common']['labels']['testServiceName'], PDO::PARAM_STR);
...
elseif ($datain == "systemsms") {
    ...
    if ($typeservice == "unpinmessage") {
        ...
        $stmt->bindValue(':time', $timenouser, PDO::PARAM_INT);
            $stmt->execute();
            $userslist = json_encode($stmt->fetchAll());
        } else {
            if ($typeusermessage == "all") {
    ...
    $sql = "SELECT * FROM Payment_report
            WHERE id_user = :id_user
              ...
              AND payment_Status != 'expire'
              AND payment_Status != 'reject'
              AND (
                  id_invoice LIKE CONCAT('%', 'getconfigafterpay', '%')
                  OR id_invoice LIKE CONCAT('%', 'getextenduser', '%')
                  OR id_invoice LIKE CONCAT('%', 'getextravolumeuser', '%')
                  OR id_invoice LIKE CONCAT('%', 'getextratimeuser', '%')
              )";
    $stmt = $pdo->prepare($sql);
    $stmt->bindValue(':id_user', (int) $Payment_report['id_user'], PDO::PARAM_INT);
    $stmt->execute();
    ...
    if (!$Balance_id) {
        telegram('answerCallbackQuery', array(
            'callback_query_id' => $callback_query_id,
            'text' => $textbotlang['Admin']['notUser'],
            'show_alert' => true,
            'cache_time' => 5,
        ));
        return;
    ...
    // Atomic claim: only the first caller to flip this order's status wins the race
    // between two near-simultaneous Confirm taps (or Confirm + Reject).
    if (!settleOrderOnce($order_id)) {
        telegram('answerCallbackQuery', array(
            'callback_query_id' => $callback_query_id,
            'text' => $textbotlang['Admin']['Payment']['reviewedPayment'],
            'show_alert' => true,
    ...
    if ($pricecashback != "0") {
        ...
        if (!creditBalance((int) $Balance_id['id'], (int) round($result))) {
            error_log("admin.php confirm payment: cashback credit failed for order {$order_id}");
        } else {
    ...
    // Note: payment_Status is already set by settleOrderOnce() above; the old
    // unconditional update("Payment_report","payment_Status","paid",...) was removed.
    ...
    if (!$Payment_report || $Payment_report['payment_Status'] !== 'reject') {
        step('home', $from_id);
        sendmessage($from_id, $textbotlang['Admin']['Payment']['reviewedPayment'], $keyboardadmin, 'HTML');
        return;
    }
    ...
    $userdata = json_decode($user['Processing_value'], true);
    // agent and price come from Processing_value and were interpolated straight
    // into the SQL. They are now bound. An unrecognised typecustomer, or the
    // agent branch's former "$query_where = null", dropped the WHERE clause
    // entirely and credited every row in the user table, so both now fail closed.
    $query_where = "";
    $params = [];
    $filterValid = true;

    $agentFilter = $userdata['agent'] ?? 'all';
    $typecustomer = $userdata['typecustomer'] ?? '';
    $amountToAdd = filter_var(
        $userdata['price'] ?? null,
        FILTER_VALIDATE_INT,
        ['options' => ['min_range' => 1, 'max_range' => 100000000]]
    );
    if ($amountToAdd === false) {
        $amountToAdd = null;
    }
    ...
    if ($agentFilter == "all") {
        ...
        } elseif ($typecustomer == "notcustomer") {
    ...
    $Balance_user = [];
    if ($filterValid && $amountToAdd !== null) {
        $stmt = $pdo->prepare("SELECT u.id FROM user u" . $query_where);
        $stmt->execute($params);
        $Balance_user = $stmt->fetchAll();
        $stmt = $pdo->prepare("UPDATE user as u SET Balance = COALESCE(Balance, 0) + :amount" . $query_where);
        $stmt->execute($params + [':amount' => $amountToAdd]);
        clearSelectCache('user');
        sendmessage($from_id, $textbotlang['Admin']['Balance']['addBalanceUsers'], $keyboardadmin, 'HTML');
    } else {
        error_log('admin.php bulk balance: refused invalid filter or amount');
        sendmessage($from_id, $textbotlang['Admin']['Balance']['invalidPrice'], $keyboardadmin, 'HTML');
    }
    step('home', $from_id);
    if ($text == "1") {
        $cancelmessage = json_encode([
    ...
    $targetUserId = (int) $user['Processing_value'];
    $amount = (int) $text;
    ...
    clearSelectCache('user');
    $balances1 = number_format($amount, 0);
    $updatedUser = select("user", "*", "id", $targetUserId, "select");
    $Balance_user_afters = number_format((int) ($updatedUser['Balance'] ?? 0));
    $textkam = sprintf($textbotlang['users']['Balance']['deductedNotice'], $balances1);
    sendmessage($targetUserId, $textkam, null, 'HTML');
    step('home', $from_id);
    if (strlen($setting['Channel_Report']) > 0) {
        $textaddbalance = sprintf($textbotlang['Admin']['reportgroup']['balanceDecreased'], $username, $from_id, $targetUserId, $amount, $Balance_user_afters);
        telegram('sendmessage', [
            'chat_id' => $setting['Channel_Report'],
            'message_thread_id' => $paymentreports,
            'text' => $textaddbalance,
    ...
    $sql = "SELECT * FROM invoice WHERE time_sell > :requestedDate AND (status = 'active' OR status = 'end_of_time'  OR status = 'end_of_volume' OR status = 'sendedwarn' OR Status = 'send_on_hold') AND name_product != :test_service_name AND id_user = :id_user";
    $stmt = $pdo->prepare($sql);
    ...
    $sql = "SELECT SUM(price_product) FROM invoice WHERE time_sell > :requestedDate AND (Status = 'active' OR Status = 'end_of_time'  OR Status = 'end_of_volume' OR status = 'sendedwarn' OR Status = 'send_on_hold') AND name_product != :test_service_name AND id_user = :id_user";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':id_user', $id_user);
    $stmt->bindParam(':requestedDate', $desired_date_time_start);
    $stmt->bindValue(':test_service_name', $textbotlang['common']['labels']['testServiceName'], PDO::PARAM_STR);
    ...
    try {
        ...
        $stmt->execute([$targetUserId, $randomString, $dateacc, $amount, $payment_Status, $Payment_Method, $invoice]);

        if (!creditBalance($targetUserId, $amount)) {
            throw new RuntimeException('Balance credit failed');
        }
        ...
        error_log('admin.php admin credit failed: ' . $e->getMessage());
        sendmessage($from_id, $textbotlang['common']['error'], $backadmin, 'HTML');
        return;
    }
    ...
    sendmessage($targetUserId, $textadd, null, 'HTML');
    step('home', $from_id);
    clearSelectCache('user');
    $Balance_user_after = number_format(select("user", "*", "id", $targetUserId, "select")['Balance']);
    $pricadd = number_format($amount);
    if (strlen($setting['Channel_Report']) > 0) {
        $textaddbalance = sprintf($textbotlang['Admin']['reportgroup']['balanceIncreased'], $username, $from_id, $targetUserId, $pricadd, $Balance_user_after);
        telegram('sendmessage', [
            'chat_id' => $setting['Channel_Report'],
            'message_thread_id' => $paymentreports,
            'text' => $textaddbalance,
    ...
    try {
        ...
        // Preserves the previous behavior of allowing this admin action to push
        // the balance negative (COALESCE keeps behavior identical if Balance is NULL).
        $balanceStmt = $pdo->prepare(
            "UPDATE user
             SET Balance = COALESCE(Balance, 0) - :amount
             WHERE id = :id"
        );
        $balanceStmt->bindValue(':amount', $amount, PDO::PARAM_INT);
        $balanceStmt->bindValue(':id', $targetUserId, PDO::PARAM_INT);
        $balanceStmt->execute();

        if ($balanceStmt->rowCount() !== 1) {
            throw new RuntimeException('User not found for balance debit');
        }
        ...
        error_log('admin.php admin debit failed: ' . $e->getMessage());
        sendmessage($from_id, $textbotlang['common']['error'], $backadmin, 'HTML');
        return;
    }
    ...
    sendmessage($targetUserId, $textkam, null, 'HTML');
    step('home', $from_id);
    $Balance_user_afters = number_format(select("user", "*", "id", $targetUserId, "select")['Balance']);
    if (strlen($setting['Channel_Report']) > 0) {
        $textaddbalance = sprintf($textbotlang['Admin']['reportgroup']['balanceDecreased2'], $username, $from_id, $targetUserId, $amount, $Balance_user_afters);
        telegram('sendmessage', [
            'chat_id' => $setting['Channel_Report'],
            'message_thread_id' => $paymentreports,
            'text' => $textaddbalance,
    ...
    $info_product = select("invoice", "*", "id_invoice", $username, "select");
    if (!$info_product) {
        sendmessage($from_id, $textbotlang['keyboard']['transactionDeleted'], $keyboardadmin, 'HTML');
        return;
    }
    // Atomic claim: only the first callback for this invoice flips its status,
    // so a duplicate tap can't remove the panel user or refund a second time.
    $claimStmt = $pdo->prepare(
        "UPDATE invoice SET status = 'removebyadmin' WHERE id_invoice = :id_invoice AND status != 'removebyadmin'"
    );
    $claimStmt->bindValue(':id_invoice', $username, PDO::PARAM_STR);
    $claimStmt->execute();
    ...
    $ManagePanel->RemoveUser($info_product['Service_location'], $info_product['username']);
    $refundAmount = (int) $info_product['price_product'];
    if ($refundAmount > 0 && !creditBalance((int) $info_product['id_user'], $refundAmount)) {
        error_log("admin.php product refund failed for invoice {$username}");
    } else {
    ...
    if (!ctype_digit($text) || (int) $text < 1 || (int) $text > 100000000) {
        sendmessage($from_id, $textbotlang['Admin']['Balance']['invalidPrice'], $backadmin, 'HTML');
        return;
    }

    $idOrder = $user['Processing_value'];
    $amount = (int) $text;
    $Payment_report = select("Payment_report", "*", "id_order", $idOrder, "select");
    ...
    try {
        ...
        if (!settleOrderOnce($idOrder)) {
            throw new RuntimeException('Payment was already settled');
        }

        if (!creditBalance((int) $Payment_report['id_user'], $amount)) {
            throw new RuntimeException('Balance credit failed');
        }
        ...
        error_log("admin.php manual balance failed for order {$idOrder}: " . $e->getMessage());
        step('home', $from_id);
        sendmessage($from_id, $textbotlang['Admin']['Payment']['reviewedPayment'], $keyboardadmin, 'HTML');
        return;
    }
    ...
    $balanceusers = number_format($amount, 0);
    $textadd = sprintf($textbotlang['users']['Balance']['addedNotice3'], $balanceusers);
    sendmessage($Payment_report['id_user'], $textadd, null, 'HTML');
    $text_report = sprintf($textbotlang['Admin']['reportgroup']['balanceManualAdd'], $Payment_report['id_user'], $Balance_user['username'], $Payment_report['price'], $amount);
    if (strlen($setting['Channel_Report']) > 0) {
        telegram('sendmessage', [
            'chat_id' => $setting['Channel_Report'],
            'message_thread_id' => $paymentreports,
    ...
    $balancemaax = json_decode($minBalanceRow['ValuePay'] ?? '{}', true);
    $balancemaax = is_array($balancemaax) ? $balancemaax : [];
    ...
    step('home', $from_id);
    $maxBalanceRow = select("PaySetting", "ValuePay", "NamePay", "maxbalance", "select");
    $balancemaax = json_decode($maxBalanceRow['ValuePay'] ?? '{}', true);
    $balancemaax = is_array($balancemaax) ? $balancemaax : [];
    $balancemaax[$text] = $user['Processing_value'];
    $balancemaax = json_encode($balancemaax);
    sendmessage($from_id, $textbotlang['Admin']['SettingnowPayment']['saveApi'], $keyboardadmin, 'HTML');
    ...
    $invoice = select("invoice", "*", "id_invoice", $id_invoice, "select");
    if (!$invoice) {
        sendmessage($from_id, $textbotlang['keyboard']['transactionDeleted'], $backadmin, 'HTML');
        return;
    }
    $requestcheck = select("cancel_service", "*", "username", $invoice['username'], "select");
    if (!$requestcheck) {
    ...
    // Atomic claim: moves this request from pending to "processing" so a second,
    // near-simultaneous tap on the same button can't remove the panel user or
    // refund twice. Finalized to "accept" further below once refund succeeds.
    $claimStmt = $pdo->prepare(
        "UPDATE cancel_service
         SET status = 'processing'
         WHERE username = :username
           AND status NOT IN ('processing', 'accept', 'reject')"
    );
    $claimStmt->bindValue(':username', $requestcheck['username'], PDO::PARAM_STR);
    $claimStmt->execute();
    if ($claimStmt->rowCount() !== 1) {
        telegram('answerCallbackQuery', array(
            'callback_query_id' => $callback_query_id,
    ...
    if (!is_array($DataUserOut) || !array_key_exists('data_limit', $DataUserOut) || !array_key_exists('expire', $DataUserOut)) {
        error_log("admin.php cancel service: invalid panel response for {$requestcheck['username']}");
        sendmessage($from_id, $textbotlang['common']['error'], $backadmin, 'HTML');
        step('home', $from_id);
        return;
    }
    ...
    if (($DataUserOut['status'] ?? null) == "on_hold") {
        $pricelast = (float) ($nameloc['price_product'] ?? 0);
    ...
    if ($pricelast != 0 && !creditBalance((int) $nameloc['id_user'], $pricelast)) {
        error_log("admin.php cancellation refund failed for user {$nameloc['id_user']}");
    } elseif ($pricelast != 0) {
    ...
    // Same atomic claim as remoceserviceadmin- above; the refund amount itself
    // is entered manually in the next step, but removal must still happen once.
    $claimStmt = $pdo->prepare(
        "UPDATE cancel_service
         SET status = 'accept'
         WHERE username = :username
    ...
    $refundAmount = (int) $text;
    if (!creditBalance((int) $invoice['id_user'], $refundAmount)) {
        error_log("admin.php manual cancellation refund failed for invoice {$user['Processing_value']}");
        sendmessage($from_id, $textbotlang['common']['error'], $backadmin, 'HTML');
        return;
    }
...
if ($datain == "settimecornremove" && $adminrulecheck['rule'] == "administrator") {
    ...
    $paymentsstars = getPaySettingValue('statusstars', 'offstars');
    $paymentstetrapay = getPaySettingValue('statustetrapay', 'offtetrapay');
    ...
    $paymentstar = [
        'onstars' => $textbotlang['Admin']['Status']['statuson'],
        'offstars' => $textbotlang['Admin']['Status']['statusoff']
    ][$paymentsstars] ?? $textbotlang['Admin']['Status']['statusoff'];
    $tetrapaystatus = [
        'ontetrapay' => $textbotlang['Admin']['Status']['statuson'],
        'offtetrapay' => $textbotlang['Admin']['Status']['statusoff']
    ][$paymentstetrapay] ?? $textbotlang['Admin']['Status']['statusoff'];
    $now_payment_status = [
        '1' => $textbotlang['Admin']['Status']['statuson'],
        '0' => $textbotlang['Admin']['Status']['statusoff']
    ...
    $Bot_Status = json_encode([
        'inline_keyboard' => [
            ...
            [
                ...
                ['text' => $paymentstar, 'callback_data' => "editpayment-stars-$paymentsstars"],
                ['text' => "⭐ Telegram Stars", 'callback_data' => "none"],
            ],
            [
                ['text' => $textbotlang['keyboard']['settings'], 'callback_data' => "tetrapaysetting"],
                ['text' => $tetrapaystatus, 'callback_data' => "editpayment-tetrapay-$paymentstetrapay"],
                ['text' => "🌐 TetraPay", 'callback_data' => "none"],
            ],
            [
    ...
    if ($type == "Cartstatus") {
        ...
        update("PaySetting", "ValuePay", $valuenew, "NamePay", "statusstars");
...
if ($datain == "settimecornday" && $adminrulecheck['rule'] == "administrator") {
    ...
    // Show the live per-Star price so the admin can sanity-check the rate
    // before selling anything.
    $liveStarRate = getStarPriceToman();
    $starsInfo = strtr($textbotlang['Admin']['Balance']['starsRateNow'], ['{rate}' => number_format($liveStarRate)]);
    sendmessage($from_id, $starsInfo . "\n\n" . $textbotlang['users']['selectoption'], $Startelegram, 'HTML');
} elseif ($datain == "tetrapaysetting" && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['users']['selectoption'], $TetraPaySetting, 'HTML');
} elseif ($text == $textbotlang['keyboard']['starsRateMode'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['Admin']['Balance']['askStarsRateMode'], $backadmin, 'HTML');
    step("getstarsratemode", $from_id);
} elseif ($user['step'] == "getstarsratemode") {
    $mode = strtolower(trim($text));
    if (!in_array($mode, ['usdt', 'ton', 'off'], true)) {
        sendmessage($from_id, $textbotlang['Admin']['Balance']['starsRateModeInvalid'], $backadmin, 'HTML');
        return;
    }
    update("PaySetting", "ValuePay", $mode, "NamePay", "stars_auto_rate");
    step("home", $from_id);
    sendmessage($from_id, $textbotlang['Admin']['Balance']['starsRateModeSaved'], $Startelegram, 'HTML');
} elseif ($text == $textbotlang['keyboard']['starsManualRate'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['Admin']['Balance']['askStarsManualRate'], $backadmin, 'HTML');
    step("getstarsmanualrate", $from_id);
} elseif ($user['step'] == "getstarsmanualrate") {
    if (!ctype_digit($text) || intval($text) < 1) {
        sendmessage($from_id, $textbotlang['common']['invalidInput'], $backadmin, 'HTML');
        return;
    }
    update("PaySetting", "ValuePay", $text, "NamePay", "stars_rate");
    step("home", $from_id);
    sendmessage($from_id, $textbotlang['Admin']['Balance']['settingSaved'], $Startelegram, 'HTML');
} elseif ($text == $textbotlang['keyboard']['starsMargin'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['Admin']['Balance']['askStarsMargin'], $backadmin, 'HTML');
    step("getstarsmargin", $from_id);
} elseif ($user['step'] == "getstarsmargin") {
    if (!ctype_digit($text)) {
        sendmessage($from_id, $textbotlang['common']['invalidInput'], $backadmin, 'HTML');
        return;
    }
    update("PaySetting", "ValuePay", $text, "NamePay", "stars_margin");
    step("home", $from_id);
    sendmessage($from_id, $textbotlang['Admin']['Balance']['settingSaved'], $Startelegram, 'HTML');
} elseif ($text == $textbotlang['keyboard']['starsMaxCountBtn'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['Admin']['Balance']['askStarsMaxCount'], $backadmin, 'HTML');
    step("getstarsmaxcount", $from_id);
} elseif ($user['step'] == "getstarsmaxcount") {
    if (!ctype_digit($text)) {
        sendmessage($from_id, $textbotlang['common']['invalidInput'], $backadmin, 'HTML');
        return;
    }
    update("PaySetting", "ValuePay", $text, "NamePay", "stars_max_count");
    step("home", $from_id);
    sendmessage($from_id, $textbotlang['Admin']['Balance']['settingSaved'], $Startelegram, 'HTML');
} elseif ($text == $textbotlang['keyboard']['starsExpiry'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['Admin']['Balance']['askStarsExpiry'], $backadmin, 'HTML');
    step("getstarsexpiry", $from_id);
} elseif ($user['step'] == "getstarsexpiry") {
    if (!ctype_digit($text) || intval($text) < 1) {
        sendmessage($from_id, $textbotlang['common']['invalidInput'], $backadmin, 'HTML');
        return;
    }
    update("PaySetting", "ValuePay", $text, "NamePay", "stars_invoice_expiry");
    step("home", $from_id);
    sendmessage($from_id, $textbotlang['Admin']['Balance']['settingSaved'], $Startelegram, 'HTML');
} elseif ($text == $textbotlang['keyboard']['tetraMerchant'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['Admin']['Balance']['askTetraMerchant'], $backadmin, 'HTML');
    step("gettetramerchant", $from_id);
} elseif ($user['step'] == "gettetramerchant") {
    $apikey = trim($text);
    if ($apikey === '' || mb_strlen($apikey) > 255) {
        sendmessage($from_id, $textbotlang['common']['invalidInput'], $backadmin, 'HTML');
        return;
    }
    update("PaySetting", "ValuePay", $apikey, "NamePay", "merchant_id_tetrapay");
    step("home", $from_id);
    sendmessage($from_id, $textbotlang['Admin']['Balance']['settingSaved'], $TetraPaySetting, 'HTML');
} elseif ($text == $textbotlang['keyboard']['tetraCashback'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['Admin']['price']['askPaymentCashback2'], $backadmin, 'HTML');
    step("gettetracashback", $from_id);
} elseif ($user['step'] == "gettetracashback") {
    if (!ctype_digit($text)) {
        sendmessage($from_id, $textbotlang['common']['invalidInput'], $backadmin, 'HTML');
        return;
    }
    ...
    step("home", $from_id);
    sendmessage($from_id, $textbotlang['Admin']['Balance']['settingSaved'], $TetraPaySetting, 'HTML');
} elseif ($text == $textbotlang['keyboard']['tetraMinAmount'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['Admin']['Balance']['askMinDeposit'], $backadmin, 'HTML');
    step("gettetramin", $from_id);
} elseif ($user['step'] == "gettetramin") {
    if (!ctype_digit($text)) {
        sendmessage($from_id, $textbotlang['common']['invalidInput'], $backadmin, 'HTML');
        return;
    }
    ...
    step("home", $from_id);
    sendmessage($from_id, $textbotlang['Admin']['Balance']['minDepositSaved'], $TetraPaySetting, 'HTML');
} elseif ($text == $textbotlang['keyboard']['tetraMaxAmount'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['Admin']['Balance']['askMaxDeposit'], $backadmin, 'HTML');
    step("gettetramax", $from_id);
} elseif ($user['step'] == "gettetramax") {
    if (!ctype_digit($text)) {
        sendmessage($from_id, $textbotlang['common']['invalidInput'], $backadmin, 'HTML');
        return;
    }
    ...
    step("home", $from_id);
    sendmessage($from_id, $textbotlang['Admin']['Balance']['maxDepositSaved'], $TetraPaySetting, 'HTML');
} elseif ($text == $textbotlang['keyboard']['tetraExpiry'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['Admin']['Balance']['askStarsExpiry'], $backadmin, 'HTML');
    step("gettetraexpiry", $from_id);
} elseif ($user['step'] == "gettetraexpiry") {
    if (!ctype_digit($text) || intval($text) < 1) {
        sendmessage($from_id, $textbotlang['common']['invalidInput'], $backadmin, 'HTML');
        return;
    }
    update("PaySetting", "ValuePay", $text, "NamePay", "tetrapay_invoice_expiry");
    step("home", $from_id);
    sendmessage($from_id, $textbotlang['Admin']['Balance']['settingSaved'], $TetraPaySetting, 'HTML');
    ...
    step("home", $from_id);
    update("PaySetting", "ValuePay", $text, "NamePay", "minbalancestars");
    ...
    step("home", $from_id);
    update("PaySetting", "ValuePay", $text, "NamePay", "maxbalancestars");
    ...
    step("home", $from_id);
    update("PaySetting", "ValuePay", $text, "NamePay", "chashbackstars");
    ...
    if (!$invocie) {
        Editmessagetext(
            $from_id,
            $message_id,
            $textbotlang['Admin']['order']['deleted'],
    ...
    $productRow = select("product", "hide_panel", "id", $user['Processing_value'], "select");
    if (!$productRow) {
        sendmessage($from_id, $textbotlang['users']['sell']['errorProduct'], $backadmin, 'HTML');
        return;
    }

    $list_panel = json_decode($productRow['hide_panel'] ?? '[]', true);
    $list_panel = is_array($list_panel) ? $list_panel : [];

    if (in_array($text, $list_panel, true)) {
        sendmessage($from_id, $textbotlang['Admin']['managepanel']['alreadyAdded'], null, 'HTML');
        return;
    }
    $list_panel[] = $text;
